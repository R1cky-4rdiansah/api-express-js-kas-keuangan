...
// import UsersMongo from "../models/userModul2.js";
...
export const Login = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { idUserLog, emailUserLog, nameUserlog, usernameUserlog },
...
export const verifyToken = (req, res, next) => {
  ...
  if (!headers)
    return res
      .status(401)
      .json({ message: "Maaf anda tidak dapat mengakses website ini" });
...
export const Logout = (req, res) => {
  ...
  jwt.sign(
    ...
    (logout, err) => {
      if (logout) return res.status(200).json({ message: "Anda telah logout" });
